...
const CleanLines = () => {
  return (
    <svg className="absolute inset-0 w-full h-full opacity-60 pointer-events-none" preserveAspectRatio="none" viewBox="0 0 1128 191">
      <path 
        d="M -100,80 C 300,30 600,150 1228,50" 
        fill="none" 
        stroke="#06ACC1" 
        strokeWidth="1.5" 
        strokeOpacity="0.4"
      />
      <path 
        d="M -100,100 C 400,150 700,50 1228,120" 
        fill="none" 
        stroke="#FF3366" 
        strokeWidth="1.5" 
        strokeOpacity="0.3"
      />
      <path 
        d="M -100,50 C 500,120 800,80 1228,140" 
        fill="none" 
        stroke="#0B1B3D" 
        strokeWidth="1.5" 
        strokeOpacity="0.25"
...
// Original proportions from personal banner
...
export default function LinkedinCompanyBannerPage() {
  ...
  const exportAsSVG = useCallback(() => {
    ...
    toSvg(bannerRef.current, { cacheBust: true, pixelRatio: 2 })
      .then((dataUrl) => {
        ...
        link.download = 'vesalius-company-banner.svg';
        link.href = dataUrl;
        link.click();
      })
  ...
  const exportAsPNG = useCallback(() => {
    ...
    toPng(bannerRef.current, { cacheBust: true, pixelRatio: 2 })
      .then((dataUrl) => {
        const link = document.createElement('a');
        link.download = 'vesalius-company-banner.png';
        link.href = dataUrl;
        link.click();
      })
      .catch((err) => console.error('oops, something went wrong!', err));
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 overflow-auto">
      ...
      {/* 
        LinkedIn Company Banner Container 
        Standard dimensions: 1128 x 191 pixels
      */}
      <div 
        ref={bannerRef}
        className="relative bg-[#FCFCFD] overflow-hidden flex-shrink-0"
        style={{ width: '1128px', height: '191px' }}
      >
        {/* Background Mesh */}
